...
async function handlePost(req, res) {
  ...
  if (!Number.isInteger(steps) || steps < 0 || steps > 100000) {
    return res.status(400).json({ ok: false, error: 'Invalid steps value' });
  }
  ...
  if (!challengeSnap.exists) {
    return res.status(404).json({ ok: false, error: 'Step challenge not found' });
  }

  const userSnap = await db.collection('users').doc(decoded.uid).get();
  const user = userSnap.exists ? userSnap.data() : {};

  await challengeRef.collection('results').doc(decoded.uid).set(
    {
      uid: decoded.uid,
