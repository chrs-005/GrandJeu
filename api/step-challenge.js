...
async function verifyUser(req) {
  ...
  try {
    ...
    return { app, db, decoded, user: snap.exists ? snap.data() : {} };
  } catch {
...
async function sendChallengePush(db, challenge) {
  ...
  const payload = JSON.stringify({
    ...
    body: `Start moving! ${Math.round(challenge.durationSeconds / 60)} min step challenge is live.`,
    url: '/app',
  });
...
async function getResults(challengeRef) {
  const snap = await challengeRef.collection('results').get();
  return snap.docs
    .map((doc) => {
      const data = doc.data();
      return {
...
async function handleGet(req, res) {
  ...
  const stateSnap = await db.collection('gameState').doc('stepChallenge').get();
  const challengeId = stateSnap.exists ? stateSnap.data().currentChallengeId : null;
  if (!challengeId) return res.status(200).json({ ok: true, challenge: null, results: [] });

  const challengeRef = db.collection('stepChallenges').doc(challengeId);
  const challenge = serializeChallenge(await challengeRef.get());
  if (!challenge) return res.status(200).json({ ok: true, challenge: null, results: [] });

  const isAdmin = user.role === 'admin';
  const results = isAdmin ? await getResults(challengeRef) : [];
  let ownResult = null;

  if (!isAdmin) {
    const ownSnap = await challengeRef.collection('results').doc(decoded.uid).get();
    if (ownSnap.exists) ownResult = ownSnap.data().steps || 0;
  }

  return res.status(200).json({ ok: true, challenge, results, ownResult });
}
...
async function handlePost(req, res) {
  ...
  const challengeRef = db.collection('stepChallenges').doc();
  const challenge = {
    id: challengeRef.id,
    active: true,
  ...
  await db.collection('gameState').doc('stepChallenge').set({
